...
lychee.define('game.state.Menu').includes([
	...
	Class.prototype = {
		...
		render: function(clock, delta) {
			...
			if (this.__current !== this.__menus['welcome']) {
		...
		setMenu: function(id) {
			...
			if (
				...
				if (id === 'welcome') {
					headline = this.game.settings.title;
				}


				this.__entities.headline.setText(headline);
					...
					if (submenu.href !== undefined) {
						...
						if (Object.prototype.toString.call(retMenu) === '[object Object]') {
							if (typeof retMenu.desc === 'string') {
								this.__entities[mId + '-' + i].setText(retMenu.desc);
							}
						}
